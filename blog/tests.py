...
class TestView(TestCase):
    ...
    def test_post_list(self):
        ...
        # 1.4 네비게이션 바가 있다.(soup.nav로 soup에 담긴 내용 중 nav요소만 가져와 navbar에 저장한다.)
        navbar = soup.nav
        # 1.5 'blog', 'about_me' 라는 문구가 내비게이션 바에 있다.
        self.assertIn('Blog', navbar.text)
        self.assertIn('About Me', navbar.text)

        # 2.1 포스트 article이 하나도 없다면
        self.assertEqual(Post.objects.count(), 0)
        # 2.2 main area 에 '아직 게시물이 없습니다.' 라는 문구가 나타난다.
        main_area = soup.find('div', id='main-area')
        self.assertIn('아직 게시물이 없습니다', main_area.text)

        # 3.1 포스트 article이 2개 있다면
        ...
        soup = BeautifulSoup(response.content, 'html.parser')
        self.assertEqual(response.status_code, 200)
        # 3.3 main area 에 article 두 개의 제목이 존재한다.
        main_area = soup.find('div', id='main-area')
